...
var settings = {
  ...
  ip: "0.0.0.0",
  securePort: 5006,
  cacheDir: '..' + path.resolve('userdata', 'cache'), //path.resolve(__dirname, 'cache'),
  webroot: '..' + path.resolve('userdata', 'static'),//path.resolve(__dirname, 'static'),
  presetDir: '..' + path.resolve('userdata', 'presets'),//path.resolve(__dirname, 'presets'),
  announceVolume: 40
};
// load user settings
...
if (!fs.existsSync(settings.presetDir + '/')) {
  fs.mkdirSync(settings.presetDir + '/');
}
